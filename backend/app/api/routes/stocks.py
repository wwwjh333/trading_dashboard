...
# ── background full init for a new ticker ────────────────────────────────────
...
async def _background_fetch_new_ticker(ticker: str) -> None:
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            cutoff = _dt.datetime.now(timezone.utc) - timedelta(hours=24)
    ...
    # 3. LLM analysis for this ticker's news
    try:
        async with AsyncSessionLocal() as db:
...
async def add_stock(
    ...
    existing = await db.execute(
        sa_select(Stock).where(Stock.user_id == current_user.id, Stock.ticker == ticker)
    )
...
async def remove_stock(
    ...
    result = await db.execute(
        sa_select(Stock).where(Stock.user_id == current_user.id, Stock.ticker == ticker.upper())
    )
...
async def get_stocks(
    ...
    result = await db.execute(
        sa_select(Stock)
        .where(Stock.user_id == current_user.id, Stock.is_active == True)  # noqa: E712
...
async def get_stock_summary(
    ...
    latest_price = price_change_pct = rsi_14 = volume = None
    if prices:
        latest = prices[0]
        ...
        if len(prices) >= 2 and prices[1].close and latest.close:
            prev, curr = float(prices[1].close), float(latest.close)
